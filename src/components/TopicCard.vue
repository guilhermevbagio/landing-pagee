  ...
  <transition name="fade-slide">
    ...
    >
      ...
      <div
        ...
        :class="{
          'border-bright': isHovered,
          'shadow-[0px_0px_28px_0px_rgba(213,57,60,0.3)]': crowned,
        }"
      >
        <div
          v-if="big"
          class="h-[50%] w-full overflow-clip relative rounded-sm border mb-4 transition-border"
        ...
        <h1
          class="flex flex-row gap-2 justify-between items-start text-start align-top text-md w-full font-milker"
          :class="[
            isHovered ? 'text-bright' : !big ? 'text-accent' : 'text-bright',
            big
              ? 'text-2xl leading-5 md:text-3xl md:leading-8 '
              : 'md:text-lg md:leading-1',
        ...
        <div
          v-if="time"
          :class="isHovered ? 'text-bright' : 'text-accent'"
          class="text-sm font-light font-poppins pt-1"
        >
          {{ time }}
        </div>
        <div
          v-if="text"
          class="min-h-48 h-48 py-2 text-sm md:text-base font-playfair"
          :class="isHovered ? 'text-bright' : 'text-accent'"
        >
          {{ text }}
        </div>
      ...
      <div v-if="linked" class="w-full px-4 mt-2 pb-0">
        <div
          class="line-appear border-b border-accent"
          :class="
            isHovered ? 'border-opacity-100 w-full' : 'border-opacity-0 w-0 '
          "
        ></div>
      </div>
    </div>
